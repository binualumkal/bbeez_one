...
import '../screens/record_list_page.dart';
...
class _GlobalSearchPageState extends State<GlobalSearchPage> {
  ...
  Future<void> loadRecords() async {
    ...
    allRecords = await locate<RecordRepository>().getAll();
  ...
  void filter(String query) {
    ...
    final grouped = <String, SearchResultItem>{};
    ...
    for (final record in allRecords) {
      ...
      if (!matched) {
        for (final field in record.fields) {
          try {
            ...
            final value = SecurityService.decrypt(
              field.value.toString(),
            ).toLowerCase();

            if (label.contains(q) || value.contains(q)) {
              matched = true;
              break;
            }
          } catch (_) {}
      ...
      grouped.update(
        ...
        (existing) => SearchResultItem(
          ...
          recordTypeName: existing.recordTypeName,
          count: existing.count + 1,
        ),
        ifAbsent: () => SearchResultItem(
  ...
  Widget buildCard(SearchResultItem item) {
    return Container(
