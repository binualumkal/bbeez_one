...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    AppLockService.instance.onLock = _redirectToLogin;
  ...
  Future<void> loadNotifications() async {
    final settingsRepository = locate<SettingsRepository>();
    final recordRepository = locate<RecordRepository>();

    final status = await settingsRepository.getBackupStatus();
    final expiring = await recordRepository.getExpiring();
    ...
    final hasExpired =
        expiring.any((r) => r.expiryDate!.isBefore(DateTime.now()));
    final hasExpiringSoon =
        expiring.any((r) => r.expiryDate!.isAfter(DateTime.now()));
    ...
    if (status['daysSince'] >= 7 || hasExpired) {
      ...
      text = hasExpired
          ...
          : (status['daysSince'] >= 999
              ? "No backup yet"
              : "Overdue: ${status['daysSince']} days");
    } else if (status['newRecords'] >= 5 ||
        status['sensitiveModified'] == true ||
        hasExpiringSoon) {
  ...
  Future<void> loadProfile() async {
    ...
    setState(() {
      ...
      profileName = name.isEmpty ? 'User' : name;
    });
  }

  Future<void> loadFavoriteRecords() async {
    final favorites = await locate<RecordRepository>().getFavorites();
  ...
  Future<void> loadDomainCounts() async {
    final names = [
      'FINANCE',
      'BUSINESS',
      'DIGITAL',
    ...
    final values = await Future.wait(
      names.map(
        locate<RecordRepository>().getCountByDomain,
      ),
    );

    final counts = Map.fromIterables(
      names,
      values,
    );
                      ...
                      Column(
                        children: [
                          ...
                          Text(
                            backupStatusText,
                            style: TextStyle(
                              color: bellColor.withValues(alpha: .7),
                              fontSize: 13,
                          ...
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (_) => InstitutionPage(
                                domainName: domain["title"] as String,
                              ),
                            ),
                          );
                        },
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                "${domainCounts[domain["title"]] ?? 0} records",
                                style: TextStyle(
                                  color: Colors.white.withValues(
                                    alpha: .55,
                                  ),
