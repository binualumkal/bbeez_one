...
class _NotificationsPageState extends State<NotificationsPage> {
  ...
  Future<void> _loadData() async {
    final statusData = await locate<SettingsRepository>().getBackupStatus();
    final records = await locate<RecordRepository>().getExpiring();
  ...
  Widget _buildContent() {
    ...
    if (status['newRecords'] >= 5) {
      alerts.add(_alertCard(
        ...
        message:
            "You have ${status['newRecords']} new records that are not yet backed up.",
      ));
    }
    ...
    if (status['sensitiveModified'] == true) {
      alerts.add(_alertCard(
        ...
        message:
            "Modifications to Finance/Personal records detected. Backup is suggested.",
      ));
    }
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.check_circle_outline,
                color: Colors.greenAccent.withValues(alpha: .5), size: 60),
