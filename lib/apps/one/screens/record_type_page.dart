...
class RecordTypePage extends StatefulWidget {
  ...
  @override
  State<RecordTypePage> createState() => _RecordTypePageState();
}

class _RecordTypePageState extends State<RecordTypePage> {
  ...
  Future<void> loadRecordTypes() async {
    final data = await locate<RecordTypeRepository>().getByDomainAndInstitution(
      domainName: widget.domainName,
      institutionName: widget.institutionName,
    );

    final unique = <String, RecordTypeModel>{};

    for (final item in data) {
      unique[item.name.toLowerCase()] = item;
    }

    if (!mounted) return;

    setState(() {
      recordTypes = unique.values.toList();

      isLoading = false;
    });
  }

  Future<void> addRecordTypeDialog() async {
        ...
        return AlertDialog(
          ...
          content: TextField(
            ...
            decoration: InputDecoration(
              hintText: "Record Type Name",
              hintStyle: TextStyle(
                color: Colors.white.withValues(alpha: .5),
              ),
              filled: true,
              fillColor: const Color(0xff18386B),
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                await locate<RecordTypeRepository>().saveByName(
                  name: newName,
                  domainName: widget.domainName,
                  institutionName: widget.institutionName,
                );
  ...
  Widget buildCard(RecordTypeModel item) {
    return Dismissible(
      key: Key(
        "rt_${item.key}",
      ),
      ...
      onDismissed: (_) async {
        await locate<RecordTypeRepository>().delete(
          item.key,
        );
      ...
      child: Container(
        ...
        child: ListTile(
          ...
          leading: Container(
            ...
            width: 55,
            decoration: BoxDecoration(
              color: (_getDomainInfo(widget.domainName)["color"] as Color)
                  .withValues(alpha: .15),
